...
- data: EulerSim{2, 4, T} object generated by Euler2D.
"""
function findShock2D(frame, data::Union{EulerSim{2,4,T}, CellBasedEulerSim{T}}) where {T}
...
"""
function findAllShocks2D(data::EulerSim{2, 4, T}) where {T}
    shock_points = []
    num_frames = Euler2D.n_tsteps(data)  

    for frame in 1:num_frames
        shock = findShock2D(frame, data)
