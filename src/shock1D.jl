...
"""
    findShock1D(frame, data::EulerSim{1, 3, T}; threshold=eps_1d) where {T}
...
function findShock1D(frame, data::EulerSim{1, 3, T}; threshold=eps_1d) where{T}
	...
	@info "Density grad max" grad_max "Threshold " threshold*grad_max
...
"""
    findAllShocks1D(data::EulerSim{1, 3, T}, threshold=eps_1d) where {T}
...
# Output:
Returns a list of lists, where each sublist contains the indices of shock points 
detected in each corresponding frame of the simulation.
"""
